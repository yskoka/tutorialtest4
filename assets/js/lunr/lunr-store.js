var store = [{
        ...
        "excerpt":"OCI Container Instances は、コンテナ用に最適化されたサーバレス・コンピューティングでアプリケーションを実行できます。OCIの管理コンソールからコンピュートシェイプ、リソース割り当て、ネットワーク構成をプロビジョニングして、複数のコンテナをローンチできます。Kubernetes をはじめとするコンテナ・オーケストレーション・プラットフォームを必要としないコンテナベースのワークロードに適したサービスです。 OCI Container Instances上でWordPress環境を構築して、実際にアプリケーションを動かす流れを体験します。 前提条件 クラウド環境 Oracle Cloudのアカウントを取得済みであること ハンズオン環境のイメージ 1.OCI Container Instancesのプロビジョニング ここでは、OCI Container Instancesのプロビジョニングを行います。 ハンバーガーメニューをクリックします。 開発者サービス-コンテナ・インスタンスを選択します。 Create container instanceボタンをクリックします。 以下画像赤枠の箇所を変更します。それ以外はデフォルト値です。 「Name」：container-instance-wordpress Networkingの下にあるShow advanced optionsをクリックします。 「Container restart policy」を以下の設定にします。 「Container restart policy」：always 次ボタンをクリックします。 WordPress のデータベースを構成する MySQL イメージを設定します。最初に、Name オプションをdbと入力します。そして、Select imageボタンをクリックします。 「Name オプション」：db External registryタブを選択して、以下の設定を行います。 「Registry hostname」：docker.io 「Repository」：mysql 「Tag...","categories": [],
        "tags": [],
        ...
        "excerpt":"0. 概要 HPC/GPUクラスタと共に利用するファイル共有ストレージは、その構築手法を決定する際に以下の評価項目を考慮する必要があります。 計算/GPUノードの複数ノード同時アクセス時性能 ランニングコスト ストレージに格納するデータの可用性 ファイル共有ストレージサービスの可用性 システム構築・運用のしやすさ 本テクニカルTipsは、HPC/GPUクラスタ向けにNFSでサービスするファイル共有ストレージを構築することを念頭に、いくつかのファイル共有ストレージ構築手法を紹介し、上記評価基準を元にどの手法を採用すればよいか、その考慮点を解説します。 NFSでサービスするファイル共有ストレージを構築する際の最初の考慮点は、マネージドのNFSサービスである ファイル・ストレージ を使用する方法と、 ベア・メタル・インスタンス とストレージサービスで構築する方法（以降”ベア・メタル・インスタンスNFSサーバ”と呼称）の、どちらを採用するかです。 ここで ファイル・ストレージ を採用する場合は、性能要件に合わせて マウント・ターゲット に標準タイプ（以降”標準FSS”と呼称）とHigh Performanceタイプ（以降”高性能FSS”と呼称）（※1）のどちらを使用するかを考慮します。 ※1）High Performanceタイプの マウント・ターゲット は、その性能違いで3種類存在しますが、本テクニカルTipsでは読み取りスループット設定が80 Gbpsで性能が最も高い HPMT-80 を前提に解説します。 High Performanceタイプの マウント・ターゲット 詳細は、 ここ を参照して下さい。 またベア・メタル・インスタンスNFSサーバを採用する場合は、以下2種類のどちらにするかを考慮します。 ストレージに ブロック・ボリューム を使用しこれをベア・メタル・インスタンスにアタッチする方法（以降”ブロック・ボリュームNFSサーバ”と呼称） ベア・メタル・インスタンス にNVMe SSDドライブを搭載するDenceIOシェイプを使用する方法（以降”DenceIO NFSサーバ”と呼称） 以上より、本テクニカルTipsで解説するファイル共有ストレージは、以下4種類です。 構築手法 使用するサービス NFSサーバ ストレージ 保存対象 データ （※2） ターゲット...","categories": [],
        "tags": [],
        ...
        "excerpt":"これはODのCloud Optimization Initiativeのページです。削除予定です。 やっぱ削除やめます。250714     ","categories": [],
        "tags": [],
